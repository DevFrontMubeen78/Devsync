...
const Portfolio = ({ title, limit, showButton = false }) => {
  const portfolioData = [
    {
      id: 1,
      title: 'Devsync',
    ...
    },
    {
      ...
      tag: 'E-Learning',
      image:
        'https://images.unsplash.com/photo-1584697964198-2e7f7f1a2a3f?auto=format&fit=crop&w=800&q=80',
      link: '#',
    },
    {
      id: 8,
    ...
    },
    {
      ...
      tag: 'FinTech',
      image:
        'https://images.unsplash.com/photo-1621504450181-5e4b3f3d0c65?auto=format&fit=crop&w=800&q=80',
      link: '#',
    },
  ]
  ...
  return (
    <div className="container flex flex-col gap-10 secondaryColor">
        ...
        {dataToShow.map((item) => (
          ...
          >
            ...
            <div className="md:w-1/2 md:p-8 p-4 flex flex-col gap-6 justify-between">
              <div className="">
                <p className="inline-block px-5 py-1 bg-[#5b696f] Text4 secondaryColor rounded-full mb-4">
                  {item.tag}
                </p>

                <h3 className="Heading4 secondaryColor mb-6">{item.title}</h3>
              </div>

              <div className="grid grid-cols-2 gap-10 text-sm md:text-base">
                <div className="flex flex-col gap-2">
                  <h4 className="Text2">Category</h4>
                  <p className="Text4">{item.category}</p>
                </div>
                <div className="flex flex-col gap-2">
                  <h4 className="Text2">Type</h4>
                  <p className="Text4">{item.type}</p>
                </div>
                <div className="flex flex-col gap-2">
                  <h4 className="Text2">Service</h4>
                  <p className="Text4">{item.service}</p>
                </div>
                <div className="flex flex-col gap-2">
                  <h4 className="Text2">Result</h4>
                  <p className="Text4">{item.result}</p>
                </div>
              </div>
      ...
      {showButton && (
        <ViewButton
          extraClass="border-white text-white"
          name="explore more our work"
          link="/portfolio"
        />
      )}
    </div>
